...
CompanyIdentifier companyIdentifiers[] = {
  { 0x0E3C
    ,"Viselabs"
...
// Function to check BLE manufacturer data
void checkManufacturerData(BLEDevice peripheral) {
    int length = peripheral.manufacturerDataLength();
    if (length > 0) {
        uint8_t manufacturerData[length];
        
        if (peripheral.manufacturerData(manufacturerData, length)) {
            // Assuming the first two bytes represent the company identifier
            if (length >= 2) {
                uint16_t companyId = (manufacturerData[1] << 8) | manufacturerData[0]; // Convert to uint16_t
                Serial.print("Company Identifier: ");
                Serial.println(companyId, HEX);

                // Get the manufacturer name
                const char* manufacturerName = getManufacturerName(companyId);
                Serial.print("Manufacturer: ");
                Serial.println(manufacturerName);
            }
        } else {
            Serial.println("Failed to read manufacturer data.");
        }
    } else {
        Serial.println("No manufacturer data available.");
    }
}

// Function to get BLE manufacturer
const char* getManufacturerName(BLEDevice peripheral) {
    int length = peripheral.manufacturerDataLength();
    if (length > 0) {
        uint8_t manufacturerData[length];
...
// Function to get the manufacturer name from identifier
const char* getManufacturerName(uint16_t id) {
    for (const auto& company : companyIdentifiers) {
        if (company.id == id) {
            return company.name;
        }
    }
